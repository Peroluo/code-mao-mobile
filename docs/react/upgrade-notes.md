...
2.0 不兼容改动，升级实例 [lbk-common-components-samples / web-1.x-2.0](https://github.com/ant-design/lbk-common-components-samples/tree/master/web-1.x-2.0)

#### 高清方案

> 如果没有使用 `lbk-common-components@1.x` 或者类似的 viewport 缩放方案则可以跳过这一步。
...
2. 参照 [自定义主题文档](https://mobile.ant.design/docs/react/customize-theme-cn) 将 lbk-common-components 提供的主题变量 `@hd` 赋值为 `2px`。
...
1. 对于 `<Icon type="loading" />` 此类使用 lbk-common-components 内置 Icon 的场景，无需任何修改。

2. 对于 `<Icon type={require('../foo.svg')} />` 此类使用本地 svg 文件的场景，可以保留原 svg-sprite-loader 相关配置不变，然后使用自定义的 `CustomIcon` 组件替换 lbk-common-components `Icon`，示例如下：

```diff
- import { Icon } from 'lbk-common-components';
...
升级示例:
  ...
  ```diff
  - import { ListView, RefreshControl } from 'lbk-common-components';
  + import { ListView, PullToRefresh } from 'lbk-common-components';
  <ListView
...
从 1.0 开始，`lbk-common-components` 严格遵循 [Semantic Versioning 2.0.0](http://semver.org/lang/zh-CN/) 语义化版本规范。
